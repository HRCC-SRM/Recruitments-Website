...
export const getCreativeStats = async (req, res) => {
  try {
    ...
    const omittedUsers = await User.countDocuments({ domain: "Creatives", status: "holded" });
...
export const sendShortlistedEmails = async (req, res) => {
  try {
    const shortlisted = await User.find({ domain: "Creatives", status: "shortlisted" });
    if (shortlisted.length === 0) {
      return res.status(200).json({ message: "No shortlisted creative users found", sent: 0 });
    }
