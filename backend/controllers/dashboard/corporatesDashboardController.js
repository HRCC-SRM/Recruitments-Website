...
export const getCorporateUsers = async (req, res) => {
  try {
    ...
    // Build query for corporate domain users
    let query = { domain: "Corporates" };
...
export const getCorporateUserById = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Corporate user retrieved successfully",
      user,
    });
  } catch (error) {
    console.error("Get corporate user error:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
...
export const sendTaskToUsers = async (req, res) => {
  try {
    ...
    // Verify all users are from corporate domain
    ...
    if (users.length !== userIds.length) {
      return res.status(400).json({ 
        message: "Some users are not from corporate domain or don't exist" 
      });
    }
    ...
    res.status(200).json({
      message: `Task sent to ${userIds.length} corporate users successfully`,
      task,
...
export const getCorporateStats = async (req, res) => {
  try {
    const totalUsers = await User.countDocuments({ domain: "Corporates" });
    ...
    const year2Users = await User.countDocuments({ domain: "Corporates", yearOfStudy: 2 });

    // Get users by branch
    const branchStats = await User.aggregate([
      { $match: { domain: "Corporates" } },
      { $group: { _id: "$branch", count: { $sum: 1 } } },
      { $sort: { count: -1 } }
    ]);

    res.status(200).json({
      message: "Corporate domain statistics retrieved successfully",
      stats: {
        totalUsers,
        statusBreakdown: {
          active: activeUsers,
...
export const bulkUpdateUserStatus = async (req, res) => {
  try {
    ...
    if (result.matchedCount === 0) {
      return res.status(404).json({ message: "No corporate users found with provided IDs" });
    }
...
// Get shortlisted corporate users
export const getShortlistedUsers = async (req, res) => {
  try {
    const { page = 1, limit = 10, search } = req.query;
    const skip = (page - 1) * limit;

    // Build query for shortlisted corporate users
    let query = { 
      domain: "Corporates",
      status: "shortlisted"
    };
    ...
    res.status(200).json({
      message: "Shortlisted corporate users retrieved successfully",
      users,
      pagination: {
        currentPage: parseInt(page),
        totalPages: Math.ceil(totalUsers / limit),
    ...
    console.error("Get shortlisted corporate users error:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};

// Send task email to shortlisted corporate users
export const sendTaskEmailToShortlisted = async (req, res) => {
  try {
    ...
    if (shortlistedUsers.length === 0) {
      return res.status(404).json({ message: "No shortlisted corporate users found" });
    }
    ...
    console.log(`Email to be sent to ${shortlistedUsers.length} shortlisted corporate users:`);
    console.log(`Subject: ${emailSubject}`);
    console.log(`Body: ${emailBody}`);
    console.log(`Task: ${taskTitle} - ${taskDescription}`);

    res.status(200).json({
      message: `Task assigned and email prepared for ${shortlistedUsers.length} shortlisted corporate users`,
      task,
      emailDetails: {
