...
import { creativesFormConfig } from "@/lib/form-configs"
import { Palette, ArrowLeft, LogOut, ChevronLeft, ChevronRight, ChevronDown } from "lucide-react"
import Link from "next/link"
import { apiClient } from "@/lib/api"
import { useRouter } from "next/navigation"
import Image from "next/image"

export default function CreativesForm() {
  ...
  const steps = creativesFormConfig.steps.map((step, index) => ({
    id: step.id,
    title: step.title,
  ...
  const currentStepConfig = creativesFormConfig.steps[currentStep]
  ...
  const handleNext = () => {
    ...
    if (currentStep < creativesFormConfig.steps.length - 1) {
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="text-center mb-8">
                <div className="mx-auto mb-4 p-4 bg-purple-500/10 rounded-full w-16 h-16 flex items-center justify-center">
                  <Palette className="h-8 w-8 text-purple-500" />
                </div>
                <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Creatives Domain Application</h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {currentStepConfig.id === "your-details" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* Inject Name and SRM Email fields which are required by backend */}
                    <div>
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4 pt-6">
                  ...
                  <div className="flex space-x-4 order-1 sm:order-2">
                    {currentStep < creativesFormConfig.steps.length - 1 ? (
                      <Button
                        type="button"
                        onClick={handleNext}
                        className="bg-purple-600 hover:bg-purple-700 flex items-center w-full sm:w-auto"
                      >
                        Next
                        <ChevronRight className="h-4 w-4 ml-2" />
                      </Button>
                    ) : (
                      <Button type="submit" className="bg-purple-600 hover:bg-purple-700 px-8 py-3 text-lg w-full sm:w-auto" disabled={isSubmitting}>
