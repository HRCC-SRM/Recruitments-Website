...
export default function CreativesAdminDashboard() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      setLoading(true)
      const response = await apiClient.getCreativeUsers({
        page: currentPage,
        limit: 50,
  ...
  const exportCsv = () => {
    ...
    link.href = url
    link.download = "creatives_applications.csv"
  ...
  return (
    <ProtectedRoute requiredRole="creative_lead" allowedDomains={["creative"]}>
      <div className="min-h-screen bg-background flex flex-col">
        <AdminNavbar title="Creatives Admin Dashboard" onLogout={handleLogout} />
