...
export default function TechnicalAdminDashboard() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      setLoading(true)
      const response = await apiClient.getTechnicalUsers({
        page: currentPage,
        limit: 50,
  ...
  // Whenever a new row is selected, reset localStatus
  useEffect(() => {
    if (selected) {
  ...
  const changeStatus = async (newStatus: 'active' | 'shortlisted' | 'rejected' | 'holded' | 'omitted') => {
    ...
    try {
      ...
      const res = await apiClient.updateTechnicalUserStatus(selected.id, { status: newStatus })
      // Update local list
  ...
  const exportCsv = () => {
    ...
    link.href = url
    link.download = "technical_applications.csv"
  ...
  return (
    <ProtectedRoute requiredRole="technical_lead" allowedDomains={["technical"]}>
      <div className="min-h-screen bg-background flex flex-col">
        <AdminNavbar title="Technical Admin Dashboard" onLogout={handleLogout} />
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const res = await fetch(`${base}/technical-dashboard/notifications/send-to-shortlisted`, {
                          method: 'POST',
                          headers: {
                ...
                <SheetContent side="right" className="sm:max-w-md">
                  ...
                  {selected && (
                    <div className="space-y-4 mt-4">
                      ...
                      <div className="pt-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button 
                            size="sm" 
                            variant="default"
                            className="ml-2 bg-blue-900 hover:bg-blue-700 text-white"
                      ...
                      <Link className="inline-flex text-sm text-primary underline" href={`/dashboard/technical/${selected.id}`}>Open full responses →</Link>
                    </div>
                  )}
                </SheetContent>
              </Sheet>
