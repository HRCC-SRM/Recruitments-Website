...
import { technicalApplications } from "@/lib/sample-applications"
import { technicalResponses } from "@/lib/sample-responses"
import Link from "next/link"
import { Button } from "@/components/ui/button"

type Params = { params: { id: string } }

export default function TechnicalApplicantResponses({ params }: Params) {
  const { id } = params
  const app = technicalApplications.find((a) => a.id === id)
  const responses = technicalResponses[id] || {}
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-border">
        <div className="container mx-auto px-4 py-6 flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold">Technical: {app.name}</h1>
